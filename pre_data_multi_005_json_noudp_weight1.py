...
df = pd.read_csv('/data/ray/pcap/pcap_dpkt_multi_dif_004.csv')
ipsrc = df['ipsrc']
ipdst = df['ipdst']
sport = df['sport']
dport = df['dport']
...
lengtemp = len(haship)
plt.figure(3)
nx.draw(G, pos=nx.circular_layout(G), node_size=[x * 9000 for x in pr.values()], node_color=[x for x in pr.values()], width=0.01, with_labels=False,alpha = 0.9)

plt.figure(4)
nx.draw(G, pos=nx.spectral_layout(G), node_size=[x * 9000 for x in pr.values()], node_color=[x * 100 for x in pr.values()], width=0.01, with_labels=False,alpha = 0.9)

plt.figure(5)
nx.draw(G, pos=nx.shell_layout(G), node_size=[x * 9000 for x in pr.values()], node_color=[x * 100 for x in pr.values()], width=0.01, with_labels=False,alpha = 0.9)
...
file_name = 'out_json_005_noudp_weight1.json' #通过扩展名指纹文件存储的数据为json格式
with open(file_name,'w') as file_object:
    json.dump(dictjson,file_object)
# dff.to_csv('C:/Users/BladeRay/Desktop/hillstone/pcap_dpkt_test_out_001.csv')
out.to_csv('/data/ray/pcap/pcap_dpkt_multi_out_005_noudp_weight1.csv')
end = time.time()
print('end')
print(end)
print(end - start)
