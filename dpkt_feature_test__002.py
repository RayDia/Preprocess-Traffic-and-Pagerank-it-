...
def dpkt_pcap():
    ...
    for ts, buf in pcap:
        ...
        if isinstance(ip.data, dpkt.tcp.TCP):
            ...
            print(http_.title)
            '''
        temppacket = socket.inet_ntoa(ip.src) + socket.inet_ntoa(ip.dst) + str(tempsport) + str(tempdport)
        if temppacket in difdictp:
            difdictp[temppacket] += len(buf)
        else:
    ...
    df.to_csv('/data/ray/pcap/pcap_dpkt_dif_001.csv')
